...
export const validateEventData = (req, res, next) => {
  const { title, description, category, priority, points, reward } = req.body;
  ...
  // Accept EITHER category OR priority
  const categoryValue = category || priority;
  if (!categoryValue || typeof categoryValue !== 'string') {
    return res.status(400).json({
      success: false,
      error: 'Invalid category/priority',
    });
  }

  // Accept EITHER points OR reward
  const pointsValue = typeof points === 'number' ? points : reward;
  if (typeof pointsValue !== 'number' || pointsValue < 0) {
    return res.status(400).json({
      success: false,
      error: 'Invalid points/reward: must be a positive number',
    });
  }

  // Normalize the data for the service
  req.body.category = categoryValue;
  req.body.points = pointsValue;
